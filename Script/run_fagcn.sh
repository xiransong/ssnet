...
#############################################
RESULTS_ROOT=$OUTPUT_ROOT"/fagcn/normal"

python ../Main/new_train_normal_gnn.py $PROJECT_ROOT --data_root $DATA_ROOT --results_root $RESULTS_ROOT \
    --model "fagcn" --config_file $CONFIG_FILE_ROOT"/common_gnn-config.yaml" \
    --from_pretrained 0 --embs_lr 0.02 --gnn_lr 0.001 --freeze_nodes_emb 1 --file_pretrained_embs $OUTPUT_ROOT"/node2vec/0/out_emb_table.pt"  \
...
#############################################
RESULTS_ROOT=$OUTPUT_ROOT"/fagcn/scaling"
